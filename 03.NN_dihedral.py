...
class DescriptorNet(nnx.Module):
    ...
    def __call__(self, edge_vecs: jnp.ndarray, Zi: jnp.ndarray, Zj: jnp.ndarray):
        env = self.env(edge_vecs)
        w = self.pair_w[Zi, Zj]           # (Ncut,)
        env = env * w[:, None]            # (Ncut, 4)
        Gi = self.embed(env[:, 0])        # (Ncut, M)
        Si = Gi.T @ env                   # (M, 4)
        Si_small = Si[:self.Mp]           # (Mp, 4)
        Di = Si @ Si_small.T              # (M, Mp)
        vecs = Si_small[:, 1:]            # (Mp, 3)
        t_list = []
        Mp = self.Mp
        for m1 in range(Mp - 2):
            for m2 in range(m1 + 1, Mp - 1):
                for m3 in range(m2 + 1, Mp):
                    a = vecs[m1]
                    b = vecs[m2]
                    c = vecs[m3]
                    # scalar triple product: (a x b) · c
                    t_val = jnp.dot(jnp.cross(a, b), c)
                    t_list.append(t_val)

        if t_list:
            Ti = jnp.stack(t_list, axis=0)   # (Tdim,)
        else:
            Ti = jnp.zeros((0,), dtype=jnp.float64)
        return Di, Ti

class FittingNet(nnx.Module):
    """
    Predict atomic energy E^i from descriptor (D^i, T^i) for a single atom.
    """
    ...
    def __call__(self, Di: jnp.ndarray, Ti: jnp.ndarray) -> jnp.ndarray:
        # flatten D^i，并与 T^i 拼接
        x = jnp.concatenate([Di.reshape(-1), Ti], axis=0)
        for lyr, g, b in zip(self.layers, self.ln_gammas, self.ln_betas):
            x = lyr(x)
            x = self._layer_norm(x, g, b)
            x = jax.nn.relu(x)
        return self.out(x)
...
class EnergyNet(nnx.Module):
    ...
    def __call__(self, edge_vecs_batch, neigh_idx_batch, Z_all):
        ...
        def _one_with_Zi(ev, ni, Zi_scalar):
            ...
            Di, Ti = self.desc(ev, Zi_scalar, Zj)
            return self.fit(Di, Ti)

        return jax.vmap(_one_with_Zi, in_axes=(0,0,0), out_axes=0)(
            edge_vecs_batch, neigh_idx_batch, Zi_batch
        )
...
def train_step(model, optimizer, edge_vecs, neigh_idx, atom2struct, E_true_struct, F_true, Z_all, pe, pf):
    def loss_fn(model):
        ...
        def ei_fn(ev, Zi, Zj):
            Di, Ti = model.desc(ev, Zi, Zj)
            return model.fit(Di, Ti).squeeze()
...
log_path = runs_dir / f"18.train_log_{run_tag}.csv"
...
test_path = runs_dir / f"18.test_metrics_{run_tag}.csv"
